...
def _trader_worker(
    ...
    try:
        ...
        # requested, so it still identifies the path if both Redis reads come up empty.
...
class SessionManager:
    ...
    def _terminate_session_worker(
        ...
        """
        Force-kill paper/zombie session workers (local registry + Redis PID + PID file).
    ...
    def start_session(cls, session_id: str, session_doc: dict, trading_logs_collection):
        ...
        try:
            ...
            try:
                cls._redis().delete(cls._session_meta_key(session_id))
            except Exception:
                pass
            raise
